...
func _on_create_lobby_button_pressed() -> void:
	print("[MultiplayerMenu][_on_create_lobby_button_pressed]: Start")
	screen_resource.request_transition("LOBBY")
	mds_webrtc_bridge.lobby_creation_requested.emit()
	print("[MultiplayerMenu][_on_create_lobby_button_pressed]: End")

func _on_join_lobby_button_pressed() -> void:
	screen_resource.request_transition("LIST_LOBBIES")
	mds_webrtc_bridge.lobbies_list_requested.emit()

func _on_player_name_changed(new_player_name: String) -> void:
	mds_webrtc_bridge.player_name_changed.emit(new_player_name)
